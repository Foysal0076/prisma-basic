...
import { Credentials } from './plugins/auth'
...
export async function isGraderOfTestResultOrAdmin(
  ...
  const { userId, isAdmin, teacherOf } = request.auth.credentials as Credentials
...
export async function isRequestedUserOrAdmin(
  ...
  const { userId, isAdmin } = request.auth.credentials as Credentials
